        ...
        <tr>
            <td valign="top">
                ...
                <table role="presentation" aria-hidden="true" cellspacing="0" cellpadding="0" border="0"
                    ...
                    <tr>
                        <td>
                            <p>We are excited to inform you that a new model account has been successfully registered on
                                your platform. Below are the details of the model.</p>
                        </td>
                    </tr>
                    <tr>
                        <td>
                    ...
                    <tr class="list">
                        <td>
                            ...
                            <p><strong>Calling Number:</strong> {{ $userInfo->calling_number }}</p>
                            <p><strong>WhatsApp Number:</strong> {{ $userInfo->whatsapp_number }}</p>
                            <p><strong>Marital Status:</strong> {{ $userInfo->marital_status }}</p>
                            <p><strong>Ethnicity:</strong> {{ $userInfo->ethnicity }}</p>
                            <p><strong>Location:</strong> {{ $userInfo->location }}</p>
                            <p><strong>Biography:</strong> {{ $userInfo->biography }}</p>
                            <p><strong>Languages Spoken:</strong>
                                {{ implode(', ', json_decode($userInfo->languages_spoken, true) )}}</p>
                            <p><strong>Driving License:</strong> {{ $userInfo->driving_license }}</p>
                            <p><strong>Email:</strong> {{ $userInfo->email }}</p>
                            <p><strong>Instagram Username:</strong> {{ $userInfo->instagram_username }}</p>
                            <p><strong>Height:</strong> {{ $userInfo->height }}</p>
                            <p><strong>Bust:</strong> {{ $userInfo->bust }}</p>
                            <p><strong>Waist:</strong> {{ $userInfo->waist }}</p>
                            <p><strong>Hip:</strong> {{ $userInfo->hip }}</p>
                            <p><strong>Weight:</strong> {{ $userInfo->weight }}</p>
                            <p><strong>Eye Color:</strong> {{ $userInfo->eye_color }}</p>
                            <p><strong>Hair Color:</strong> {{ $userInfo->hair_color }}</p>
                            <p><strong>Hair Length:</strong> {{ $userInfo->hair_length }}</p>
                            <p><strong>Shoe Size:</strong> {{ $userInfo->shoe_size }}</p>
                            <p><strong>Dress Size:</strong> {{ $userInfo->dress_size }}</p>
                            <p><strong>Hourly Rate:</strong> {{ $userInfo->hourly_rate }}</p>
                            <p><strong>Session Rate:</strong> {{ $userInfo->session_rate }}</p>
                            <p><strong>Categories:</strong> {{ implode(', ', json_decode($userInfo->category, true)) }}
                            </p>
                            <p><strong>Musician Categories:</strong>
                                {{ implode(', ', json_decode($userInfo->musician_categories, true)) }}</p>
                            <p><strong>Have Tattoos:</strong> {{ $userInfo->have_tattos }}</p>

                            {{-- @if (!empty($userInfo->profile_images))
                                <h3>Profile Images:</h3>
                                @foreach (json_decode($userInfo->profile_images, true) as $image)
                                    <img src="{{ asset('uploads/models/' . $image) }}" alt="Profile Image"
