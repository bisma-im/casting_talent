<section class="testtimonailsec" data-aos="fade-down" data-aos-easing="linear" data-aos-duration="1500">
    <div class="container">
        <div class="testimonailbox">
            <div class="row">
                <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 col-xxl-12">
                    <div class="testtimonailtext">
                        <h2>What our <span>Clients have</span> to Say</h2>
                    </div>
                    <!--<div class="row align-items-center">-->
                    <!--    <div class="col-12 col-sm-12 col-md-6 col-lg-6 col-xl-6 col-xxl-6">-->
                    <!--        <div class="testimonailimg">-->
                    <!--            <img src="{{ url('user-assets') }}/images/testimonalimg.png" class="img-fluid"-->
                    <!--                alt="img">-->
                    <!--        </div>-->
                    <!--    </div>-->
                    <!--    <div class="col-12 col-sm-12 col-md-6 col-lg-6 col-xl-6 col-xxl-6">-->
                    <!--        <div class="owl-carousel owl-theme" id="testimonailSlider">-->
                    <!--            <div class="item">-->
                    <!--                <div class="testimonailSlider">-->
                    <!--                    <p>“It was popularised in the 1960s with the release of Letraset sheets-->
                    <!--                        containing Lorem Ipsum passages, and more recently with desktop publishing-->
                    <!--                        software like Aldus PageMaker including versions of Lorem Ipsum.”</p>-->
                    <!--                    <h4>DAVID JOHNSON</h4>-->
                    <!--                    <h5>CEO - FOUNDER</h5>-->
                    <!--                </div>-->
                    <!--            </div>-->
                    <!--            <div class="item">-->
                    <!--                <div class="testimonailSlider">-->
                    <!--                    <p>“It was popularised in the 1960s with the release of Letraset sheets-->
                    <!--                        containing Lorem Ipsum passages, and more recently with desktop publishing-->
                    <!--                        software like Aldus PageMaker including versions of Lorem Ipsum.”</p>-->
                    <!--                    <h4>DAVID JOHNSON</h4>-->
                    <!--                    <h5>CEO - FOUNDER</h5>-->
                    <!--                </div>-->
                    <!--            </div>-->
                    <!--            <div class="item">-->
                    <!--                <div class="testimonailSlider">-->
                    <!--                    <p>“It was popularised in the 1960s with the release of Letraset sheets-->
                    <!--                        containing Lorem Ipsum passages, and more recently with desktop publishing-->
                    <!--                        software like Aldus PageMaker including versions of Lorem Ipsum.”</p>-->
                    <!--                    <h4>DAVID JOHNSON</h4>-->
                    <!--                    <h5>CEO - FOUNDER</h5>-->
                    <!--                </div>-->
                    <!--            </div>-->
                    <!--        </div>-->
                    <!--    </div>-->
                    <!--</div>-->
                    <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 col-xxl-12 mt-5">
        <div class="owl-carousel owl-theme" id="testimonailSlider">
            <div class="item">
                <div class="testimonailSlider d-flex align-items-center">
                    <div class="" style="width: 700px; ">
                        <img src="{{ url('user-assets') }}/images/testimonalimg.png" class="img-fluid" alt="img" style="width: 100%;">
                    </div>
                    <div class="p-3">
                        <p>“It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.”</p>
                        <h4>DAVID JOHNSON</h4>
                        <h5>CEO - FOUNDER</h5>
                    </div>
                </div>
            </div>
            <div class="item">
                <div class="testimonailSlider d-flex align-items-center">
                    <div class="" style="width: 700px; ">
                        <img src="{{ url('user-assets') }}/images/testimonalimg.png" class="img-fluid" alt="img" style="width: 100%;">
                    </div>
                    <div class="p-3">
                        <p>“It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.”</p>
                        <h4>DAVID JOHNSON</h4>
                        <h5>CEO - FOUNDER</h5>
                    </div>
                </div>
            </div>
            <div class="item">
                <div class="testimonailSlider d-flex align-items-center">
                    <div class="" style="width: 700px; ">
                        <img src="{{ url('user-assets') }}/images/testimonalimg.png" class="img-fluid" alt="img" style="width: 100%;">
                    </div>
                    <div class="p-3">
                        <p>“It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.”</p>
                        <h4>DAVID JOHNSON</h4>
                        <h5>CEO - FOUNDER</h5>
                    </div>
                </div>
            </div>
        </div>
    </div>
                </div>
            </div>
        </div>
    </div>
</section>


<!--<section class="ourclientssec" data-aos="fade-down" data-aos-easing="linear" data-aos-duration="1500">-->
<!--    <div class="container">-->
<!--        <div class="clientbox">-->
<!--            <div class="row align-items-center">-->
<!--                <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 col-xxl-12">-->
<!--                    <div class="clientsbox">-->
<!--                        <h3>Our <span>Clients</span></h3>-->
<!--                    </div>-->
<!--                    <div class="clientlist">-->
<!--                        <ul>-->
<!--                            <li><img src="{{ url('user-assets') }}/images/client_1.png" class="img-fluid"></li>-->
<!--                            <li><img src="{{ url('user-assets') }}/images/client_2.png" class="img-fluid"></li>-->
<!--                            <li><img src="{{ url('user-assets') }}/images/client_3.png" class="img-fluid"></li>-->
<!--                            <li><img src="{{ url('user-assets') }}/images/client_4.png" class="img-fluid"></li>-->
<!--                            <li><img src="{{ url('user-assets') }}/images/client_5.png" class="img-fluid"></li>-->
<!--                            <li><img src="{{ url('user-assets') }}/images/client_6.png" class="img-fluid"></li>-->
<!--                        </ul>-->
<!--                    </div>-->
<!--                </div>-->
<!--            </div>-->
<!--        </div>-->
<!--    </div>-->
<!--</section>-->

<section class="ourclientssec" data-aos="fade-down" data-aos-easing="linear" data-aos-duration="1500">
    <div class="container ">
        <div class="clientbox">
            <div class="row align-items-center">
                <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 col-xxl-12">
                    <div class="clientsbox">
                        <h3>Our <span>Clients</span></h3>
                    </div>
                    <!--<div class="owl-carousel owl-theme mt-5" id="testimonailSliderff">-->
                    <!--    <div class="item">-->
                    <!--        <div class="image-container">-->
                    <!--            <img src="{{ url('user-assets/loggs/1.png') }}" class="img-fluid" alt="Image 1" style="width: 150px; height: 70px;">-->
                    <!--        </div>-->
                    <!--    </div>-->
                    <!--    <div class="item">-->
                    <!--        <div class="image-container">-->
                    <!--            <img src="{{ url('user-assets/loggs/2.png') }}" class="img-fluid" alt="Image 2" style="width: 150px; height: 70px;">-->
                    <!--        </div>-->
                    <!--    </div>-->
                    <!--    <div class="item">-->
                    <!--        <div class="image-container">-->
                    <!--            <img src="{{ url('user-assets/loggs/3.png') }}" class="img-fluid" alt="Image 3" style="width: 150px; height: 70px;">-->
                    <!--        </div>-->
                    <!--    </div>-->
                    <!--    <div class="item">-->
                    <!--        <div class="image-container">-->
                    <!--            <img src="{{ url('user-assets/loggs/4.png') }}" class="img-fluid" alt="Image 4" style="width: 150px; height: 70px;">-->
                    <!--        </div>-->
                    <!--    </div>-->
                    <!--    <div class="item">-->
                    <!--        <div class="image-container">-->
                    <!--            <img src="{{ url('user-assets/loggs/5.png') }}" class="img-fluid" alt="Image 5" style="width: 150px; height: 70px;">-->
                    <!--        </div>-->
                    <!--    </div>-->
                    <!--    <div class="item">-->
                    <!--        <div class="image-container">-->
                    <!--            <img src="{{ url('user-assets/loggs/6.png') }}" class="img-fluid" alt="Image 6" style="width: 150px; height: 70px;">-->
                    <!--        </div>-->
                    <!--    </div>-->
                    <!--    <div class="item">-->
                    <!--        <div class="image-container">-->
                    <!--            <img src="{{ url('user-assets/loggs/7.png') }}" class="img-fluid" alt="Image 7" style="width: 150px; height: 70px;">-->
                    <!--        </div>-->
                    <!--    </div>-->
                    <!--    <div class="item">-->
                    <!--        <div class="image-container">-->
                    <!--            <img src="{{ url('user-assets/loggs/8.png') }}" class="img-fluid" alt="Image 8" style="width: 150px; height: 70px;">-->
                    <!--        </div>-->
                    <!--    </div>-->
                    <!--    <div class="item">-->
                    <!--        <div class="image-container">-->
                    <!--            <img src="{{ url('user-assets/loggs/9.png') }}" class="img-fluid" alt="Image 9" style="width: 150px; height: 70px;">-->
                    <!--        </div>-->
                    <!--    </div>-->
                    <!--    <div class="item">-->
                    <!--        <div class="image-container">-->
                    <!--            <img src="{{ url('user-assets/loggs/10.png') }}" class="img-fluid" alt="Image 10" style="width: 150px; height: 70px;">-->
                    <!--        </div>-->
                    <!--    </div>-->
                    <!--    <div class="item">-->
                    <!--        <div class="image-container">-->
                    <!--            <img src="{{ url('user-assets/loggs/11.png') }}" class="img-fluid" alt="Image 11" style="width: 150px; height: 70px;">-->
                    <!--        </div>-->
                    <!--    </div>-->
                    <!--    <div class="item">-->
                    <!--        <div class="image-container">-->
                    <!--            <img src="{{ url('user-assets/loggs/12.png') }}" class="img-fluid" alt="Image 12" style="width: 150px; height: 70px;">-->
                    <!--        </div>-->
                    <!--    </div>-->
                    <!--    <div class="item">-->
                    <!--        <div class="image-container">-->
                    <!--            <img src="{{ url('user-assets/loggs/13.webp') }}" class="img-fluid" alt="Image 13" style="width: 150px; height: 70px;">-->
                    <!--        </div>-->
                    <!--    </div>-->
                    <!--    <div class="item">-->
                    <!--        <div class="image-container">-->
                    <!--            <img src="{{ url('user-assets/loggs/141.png') }}" class="img-fluid" alt="Image 14" style="width: 150px; height: 70px;">-->
                    <!--        </div>-->
                    <!--    </div>-->
                    <!--    <div class="item">-->
                    <!--        <div class="image-container">-->
                    <!--            <img src="{{ url('user-assets/loggs/15.png') }}" class="img-fluid" alt="Image 15" style="width: 150px; height: 70px;">-->
                    <!--        </div>-->
                    <!--    </div>-->
                    <!--</div>-->
                    <div class="owl-carousel owl-theme mt-5" id="testimonailSliderff">
                        @for ($i = 1; $i <= 44; $i++)
                            <div class="item">
                                <div class="image-container">
                                    <img src="{{ url('user-assets/loggs/' . $i . '.png') }}" class="img-fluid" alt="Image {{ $i }}" style="width: 100%;">
                                </div>
                            </div>
                        @endfor
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>

<footer>
    <div class="container">
        <div class="footermain">
            <div class="row align-items-center">
                <div class="col-12 col-sm-12 col-md-2 col-lg-2 col-xl-2 col-xxl-2">
                    <div class="footerlogo">
                        <img src="{{ url('user-assets') }}/images/Logo.png" class="img-fluid" alt="img">
                    </div>
                </div>
                <div class="col-12 col-sm-12 col-md-8 col-lg-8 col-xl-8 col-xxl-8">
                    <div class="footerlist">
                        <ul>
                            <li><a href="{{ route('index.get') }}" class="active">HOME</a></li>
                            <li><a href="{{ route('about.get') }}">ABOUT US</a></li>
                            <li><a href="{{ route('services.get') }}">SERVICES</a></li>
                            <li><a href="{{ route('jobs.get') }}">JOBS</a></li>
                            <li><a href="{{ route('talents.get') }}">TALENTS</a></li>
                            <li><a href="{{ route('featured-models.get') }}">FEATURED MODELS</a></li>
                        </ul>
                    </div>
                </div>
                <div class="col-12 col-sm-12 col-md-2 col-lg-2 col-xl-2 col-xxl-2">
                    <div class="socaillisnk">
                        <ul>
                            <li><a href="#"><i class="fa-brands fa-snapchat"></i></a></li>
                            <li><a href="#"><i class="fa-brands fa-facebook"></i></a></li>
                            <li><a href="#"><i class="fa-brands fa-linkedin-in"></i></a></li>
                            <li><a href="#"><i class="fa-brands fa-instagram"></i></a></li>
                        </ul>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 col-xxl-12">
                    <div class="copytext">
                        <p>© casttalents.com 2024. All rights reserved</p>
                    </div>
                </div>
            </div>
        </div>
    </div>

</footer>


<!-- jQuery Library (Make sure it's loaded first) -->
<script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.6.0/jquery.min.js"></script>

<!-- Select2 JS -->
<script src="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/js/select2.min.js"></script>

<!-- Select2 CSS -->
<link href="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/css/select2.min.css" rel="stylesheet" />



<!--<script src="{{ url('user-assets') }}/js/jquery-3.6.0.min.js"></script>-->
<script src="{{ url('user-assets') }}/js/bootstrap.min.js"></script>
<!--<script src="https://unpkg.com/tilt.js@1.1.21/dest/tilt.jquery.min.js"></script>-->
<!--<script src="https://cdnjs.cloudflare.com/ajax/libs/parallax/3.1.0/parallax.min.js"></script>-->
<!--<script src="https://unpkg.com/@lottiefiles/lottie-player@latest/dist/lottie-player.js"></script>-->
<!--<script src="https://cdnjs.cloudflare.com/ajax/libs/gsap/1.20.2/TweenMax.min.js"></script>-->
<!--<script src="{{ url('user-assets') }}/js/gsap.js"></script>-->
<!--<script src="{{ url('user-assets') }}/js/ScrollSmoother.js"></script>-->
<!--<script src="{{ url('user-assets') }}/js/ScrollToPlugin.min.js"></script>-->
<!--<script src="{{ url('user-assets') }}/js/ScrollTrigger.min.js"></script>-->
<!--<script src="{{ url('user-assets') }}/js/CustomEase.min.js"></script>-->
<!--<script src="https://cdn.jsdelivr.net/npm/@fancyapps/ui@4.0/dist/fancybox.umd.js"></script>-->
<script src="https://cdnjs.cloudflare.com/ajax/libs/OwlCarousel2/2.3.4/owl.carousel.js"></script>
<script src="https://unpkg.com/aos@2.3.1/dist/aos.js"></script>
<!--<script src="{{ url('user-assets') }}/js/lazyload.min.js"></script> -->
<script src="{{ url('user-assets') }}/js/custom.js"></script>

<script>
    AOS.init();
</script>


<script>
    $(document).ready(function(){
    $("#testimonailSliderff").owlCarousel({
        loop: true,
        margin: 10,
        nav: true,
        dots: false,
        autoplay: true,
        autoplayTimeout: 3000,
        autoplayHoverPause: true,
        responsive: {
            0: {
                items: 2
            },
            600: {
                items: 4
            },
            1000: {
                items: 6
            }
        }
    });
});

</script>



</body>

</html>
